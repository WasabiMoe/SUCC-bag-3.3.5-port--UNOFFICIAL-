-- ===== SUCC-bag safety wrapper for unpack (prevents unpack(nil) crashes) =====
-- Save original unpack and override with a safe version for this addon.
if not SUCC__orig_unpack then
    SUCC__orig_unpack = unpack
    unpack = function(tbl, ...)
        if type(tbl) ~= "table" then
            return nil
        end
        return SUCC__orig_unpack(tbl, ...)
    end
end
-- ===== end unpack wrapper =====

-- ===== SUCC-bag compatibility shim for WoW 3.3.5 =====
if not Container then Container = {} end

local function safeGetContainerItemInfo(bag, slot)
...
-- preserve any existing Container.GetItemInfo
local _existing_CGI = Container.GetItemInfo
function Container.GetItemInfo(bag, slot)
    -- prefer existing implementation if it returns a table
    if _existing_CGI then
        local ok = _existing_CGI(bag, slot)
        if type(ok) == "table" then
            return ok
        end
    end
    local texture, count, locked, quality, readable, lootable, link = safeGetContainerItemInfo(bag, slot)
    if not texture and not link then
        -- return an empty table (not nil) to avoid unpack(nil)
        return {}
    end
    return {
        iconFileID = texture,
        ...
        quality = quality,
        hyperlink = link or safeGetContainerItemLink(bag, slot)
    }
end

function Container.GetNumSlots(bag)
    return safeGetContainerNumSlots(bag)
end

function Container.GetItemLink(bag, slot)
    return safeGetContainerItemLink(bag, slot)
end

function Container.UseItem(bag, slot)
    if UseContainerItem then UseContainerItem(bag, slot) end
end
...
-- ====== Patch: Fix for GameTooltip:SetOwner(nil) crash in 3.3.5 ======
local orig_SetOwner = GameTooltip.SetOwner
function GameTooltip:SetOwner(owner, ...)
    if not owner then
        -- fallback: anchor tooltip to UIParent instead of crashing
        return orig_SetOwner(self, UIParent, "ANCHOR_CURSOR")
    end
    return orig_SetOwner(self, owner, ...)
end
-- ====== end fix ======

-- ====== 3.3.5 Safe Tooltip & BagSlot fix (prevents nil self/owner crashes) ======
local _orig_BagSlotButton_OnEnter = BagSlotButton_OnEnter
BagSlotButton_OnEnter = function(self, ...)
    if not self then
        -- If somehow called manually or from SUCC-bag without a frame, ignore safely
        return
    end
    return _orig_BagSlotButton_OnEnter(self, ...)
end

local _orig_GameTooltip_SetOwner = GameTooltip.SetOwner
function GameTooltip:SetOwner(owner, ...)
    if not owner or type(owner) ~= "table" then
        -- fallback: attach to UIParent or cursor, not nil
        return _orig_GameTooltip_SetOwner(self, UIParent, "ANCHOR_CURSOR")
    end
    return _orig_GameTooltip_SetOwner(self, owner, ...)
end
